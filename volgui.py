...
class VolatilityGUI(QMainWindow):
    ...
    def _create_ui(self):
        ...
        cg.setMinimumHeight(220)
        self.config_form = PluginConfigForm(); ci.addWidget(self.config_form)
        ctrl = QHBoxLayout(); ctrl.setSpacing(6)
        self.run_btn = QPushButton("▶ RUN"); self.run_btn.setObjectName("runButton"); self.run_btn.clicked.connect(self._run_analysis); self.run_btn.setEnabled(False); self.run_btn.setMinimumHeight(32); ctrl.addWidget(self.run_btn)
        self.stop_btn = QPushButton("⏹ STOP"); self.stop_btn.setObjectName("stopButton"); self.stop_btn.clicked.connect(self._stop_analysis); self.stop_btn.setEnabled(False); self.stop_btn.setFixedWidth(80); self.stop_btn.setMinimumHeight(32); ctrl.addWidget(self.stop_btn)
        ci.addLayout(ctrl)
