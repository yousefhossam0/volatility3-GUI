...
COLORS = {
    ...
    "bg_dark":        "#0a0f1e",
    "bg_surface":     "#0f1629",
    "bg_surface_alt": "#131c32",
    "bg_hover":       "#182844",
    "bg_selected":    "#163556",
    "bg_glass":       "rgba(15,22,41,0.85)",
    "primary":        "#22d3ee",
    "primary_dim":    "#0891b2",
    "primary_glow":   "rgba(34,211,238,0.12)",
    "secondary":      "#a78bfa",
    "accent":         "#67e8f9",
    "success":        "#10b981",
    "warning":        "#f59e0b",
    ...
    "border_light":   "#2a3a5c",
    "border_focus":   "#22d3ee",
    "row_suspicious": "#2a1008",
    "row_injected":   "#1a0828",
    "status_normal":  "#0d3320",
    "status_suspicious": "#3a1a08",
    "status_injected": "#2a0838",
}
...
QMenuBar::item {{ padding: 7px 16px; border-radius: 5px; margin: 2px 2px; }}
...
QMenu {{
    ...
    border: 1px solid {COLORS['border_light']}; border-radius: 10px; padding: 8px 0px;
}}
QMenu::item {{ padding: 9px 36px 9px 22px; border-radius: 4px; margin: 1px 6px; }}
QMenu::item:selected {{ background-color: {COLORS['bg_hover']}; color: {COLORS['primary']}; }}
QMenu::separator {{ height: 1px; background-color: {COLORS['border']}; margin: 5px 12px; }}
QToolBar {{
    background-color: {COLORS['bg_darkest']}; border-bottom: 1px solid {COLORS['border']};
    padding: 5px 10px; spacing: 5px;
}}
QToolBar::separator {{ width: 1px; background-color: {COLORS['border']}; margin: 5px 8px; }}
QToolButton {{
    background-color: transparent; color: {COLORS['text_secondary']};
    border: 1px solid transparent; border-radius: 7px; padding: 8px 16px;
...
QStatusBar {{
    ...
    border-top: 1px solid {COLORS['border']}; font-size: 12px; padding: 3px 12px;
}}
QSplitter::handle {{ background-color: {COLORS['border']}; margin: 1px; border-radius: 1px; }}
QSplitter::handle:horizontal {{ width: 3px; }}
QSplitter::handle:vertical {{ height: 3px; }}
QSplitter::handle:hover {{ background-color: {COLORS['primary']}; }}
QTreeWidget {{
    background-color: {COLORS['bg_surface']}; color: {COLORS['text_primary']};
    border: 1px solid {COLORS['border']}; border-radius: 10px; padding: 6px;
    outline: none; font-size: 12px;
}}
QTreeWidget::item {{ padding: 6px 10px; border-radius: 5px; margin: 1px 2px; }}
QTreeWidget::item:hover {{ background-color: {COLORS['bg_hover']}; }}
QTreeWidget::item:selected {{ background-color: {COLORS['bg_selected']}; color: {COLORS['accent']}; border-left: 3px solid {COLORS['primary']}; }}
QTreeWidget::branch {{ background-color: transparent; }}
QTreeWidget QHeaderView::section {{
    background-color: {COLORS['bg_darkest']}; color: {COLORS['text_muted']};
    border: none; border-bottom: 1px solid {COLORS['border']}; padding: 8px 12px;
    font-weight: 700; font-size: 10px; text-transform: uppercase; letter-spacing: 1.2px;
}}
QTableWidget {{
    background-color: {COLORS['bg_surface']}; color: {COLORS['text_primary']};
...
QTableWidget::item {{ padding: 5px 10px; border-bottom: 1px solid {COLORS['border']}; }}
...
QHeaderView::section {{
    ...
    border-right: 1px solid {COLORS['border']}; padding: 10px 12px;
...
QTabWidget::pane {{
    ...
    border-radius: 0px 0px 10px 10px; background-color: {COLORS['bg_surface']};
}}
QTabBar::tab {{
    background-color: {COLORS['bg_surface_alt']}; color: {COLORS['text_secondary']};
    border: 1px solid {COLORS['border']}; border-bottom: none;
    border-radius: 8px 8px 0px 0px; padding: 9px 22px; margin-right: 3px;
    font-weight: 600; font-size: 12px;
}}
QTabBar::tab:hover {{ background-color: {COLORS['bg_hover']}; color: {COLORS['text_primary']}; }}
QTabBar::tab:selected {{ background-color: {COLORS['bg_surface']}; color: {COLORS['primary']}; border-bottom: 2px solid {COLORS['primary']}; font-weight: 700; }}
QTextEdit, QPlainTextEdit {{
    background-color: {COLORS['bg_surface']}; color: {COLORS['text_secondary']};
    border: 1px solid {COLORS['border']}; border-radius: 10px; padding: 10px;
    font-family: 'Consolas', 'JetBrains Mono', 'Cascadia Code', monospace; font-size: 12px;
    selection-background-color: {COLORS['bg_selected']};
}}
QLineEdit {{
    background-color: {COLORS['bg_surface']}; color: {COLORS['text_primary']};
    border: 1px solid {COLORS['border']}; border-radius: 8px; padding: 9px 14px;
    font-size: 13px; selection-background-color: {COLORS['primary_dim']};
}}
QLineEdit:focus {{ border: 2px solid {COLORS['border_focus']}; background-color: {COLORS['bg_surface_alt']}; padding: 8px 13px; }}
QLineEdit::placeholder {{ color: {COLORS['text_muted']}; }}
QPushButton {{
    background-color: {COLORS['bg_surface_alt']}; color: {COLORS['text_primary']};
    border: 1px solid {COLORS['border_light']}; border-radius: 8px; padding: 9px 22px;
...
QPushButton#runButton {{
    ...
    color: {COLORS['bg_darkest']}; border: none; font-weight: 800; font-size: 14px;
...
QPushButton#stopButton {{
    ...
    font-weight: 800; border-radius: 10px; padding: 12px 28px;
}}
QPushButton#stopButton:hover {{ background-color: {COLORS['danger']}; }}
QProgressBar {{
    background-color: {COLORS['bg_darkest']}; border: 1px solid {COLORS['border']};
    border-radius: 8px; text-align: center; color: {COLORS['text_secondary']};
    font-size: 11px; font-weight: 600; height: 24px;
}}
QProgressBar::chunk {{
    background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 {COLORS['primary_dim']}, stop:0.5 {COLORS['primary']}, stop:1 {COLORS['accent']});
    border-radius: 7px;
}}
QGroupBox {{
    background-color: {COLORS['bg_surface']}; border: 1px solid {COLORS['border']};
    border-radius: 10px; margin-top: 18px; padding: 18px; padding-top: 32px;
    font-weight: 700; font-size: 12px; color: {COLORS['primary']};
}}
QGroupBox::title {{
    subcontrol-origin: margin; subcontrol-position: top left;
    padding: 5px 14px; color: {COLORS['primary']}; font-size: 11px; letter-spacing: 1px;
    text-transform: uppercase;
}}
QScrollBar:vertical {{
    background-color: transparent; width: 8px; border-radius: 4px; margin: 2px;
}}
...
QScrollBar:horizontal {{
    background-color: transparent; height: 8px; border-radius: 4px; margin: 2px;
}}
QScrollBar::handle:horizontal {{ background-color: {COLORS['border_light']}; border-radius: 4px; min-width: 30px; }}
QScrollBar::handle:horizontal:hover {{ background-color: {COLORS['text_muted']}; }}
QScrollBar::add-line:horizontal, QScrollBar::sub-line:horizontal {{ width: 0px; }}
QComboBox {{
    background-color: {COLORS['bg_surface']}; color: {COLORS['text_primary']};
    border: 1px solid {COLORS['border']}; border-radius: 8px; padding: 8px 14px; font-size: 13px;
}}
QComboBox:hover {{ border-color: {COLORS['primary']}; }}
QComboBox::drop-down {{ border: none; padding-right: 10px; }}
QComboBox QAbstractItemView {{
    background-color: {COLORS['bg_surface']}; color: {COLORS['text_primary']};
    border: 1px solid {COLORS['border_light']}; selection-background-color: {COLORS['bg_selected']};
    selection-color: {COLORS['accent']}; outline: none; border-radius: 8px;
}}
QSpinBox {{
    background-color: {COLORS['bg_surface']}; color: {COLORS['text_primary']};
    border: 1px solid {COLORS['border']}; border-radius: 8px; padding: 8px 14px; font-size: 13px;
...
QCheckBox {{ color: {COLORS['text_primary']}; spacing: 10px; font-size: 13px; }}
QCheckBox::indicator {{
...
QLabel#sectionTitle {{ color: {COLORS['primary']}; font-size: 12px; font-weight: 800; letter-spacing: 1.5px; padding: 10px 0px; text-transform: uppercase; }}
...
QFrame#separator {{ background-color: {COLORS['border']}; max-height: 1px; margin: 10px 0px; }}
QDialog {{ background-color: {COLORS['bg_dark']}; border: 1px solid {COLORS['border_light']}; border-radius: 14px; }}
QToolTip {{
    background-color: {COLORS['bg_surface_alt']}; color: {COLORS['text_primary']};
    border: 1px solid {COLORS['border_light']}; border-radius: 8px; padding: 10px 14px; font-size: 12px;
}}
"""
...
class SpinnerAnimation:
    """Braille spinner for animated progress text."""
    FRAMES = "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
    def __init__(self): self._idx = 0
    def next(self): c = self.FRAMES[self._idx % len(self.FRAMES)]; self._idx += 1; return c
    def reset(self): self._idx = 0


class SessionStatsWidget(QWidget):
    """Sidebar widget showing session statistics like the reference design."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self._setup_ui()
        self.total_runs = 0; self.total_results = 0; self.anomalies = 0; self.duration_secs = 0

    def _setup_ui(self):
        self.setStyleSheet(f"background-color: {COLORS['bg_surface']}; border: 1px solid {COLORS['border']}; border-radius: 10px;")
        layout = QVBoxLayout(self); layout.setContentsMargins(12, 10, 12, 10); layout.setSpacing(6)
        title = QLabel("SESSION STATS"); title.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 10px; font-weight: 700; letter-spacing: 1.5px; border: none; background: transparent;")
        layout.addWidget(title)
        grid = QGridLayout(); grid.setSpacing(8)
        self.runs_val = QLabel("0"); self.results_val = QLabel("0")
        self.anomalies_val = QLabel("0"); self.duration_val = QLabel("0s")
        for i, (label, val_widget) in enumerate([("Runs", self.runs_val), ("Results", self.results_val), ("Anomalies", self.anomalies_val), ("Duration", self.duration_val)]):
            row, col = divmod(i, 2)
            val_widget.setStyleSheet(f"color: {COLORS['text_primary']}; font-size: 18px; font-weight: 800; border: none; background: transparent;")
            lbl = QLabel(label); lbl.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 10px; font-weight: 600; border: none; background: transparent;")
            cell = QVBoxLayout(); cell.setSpacing(0); cell.addWidget(val_widget); cell.addWidget(lbl)
            grid.addLayout(cell, row, col)
        layout.addLayout(grid)

    def update_stats(self, runs=None, results=None, anomalies=None, duration=None):
        if runs is not None: self.total_runs = runs; self.runs_val.setText(str(runs))
        if results is not None: self.total_results = results; self.results_val.setText(f"{results:,}")
        if anomalies is not None:
            self.anomalies = anomalies; self.anomalies_val.setText(str(anomalies))
            color = COLORS['error'] if anomalies > 0 else COLORS['text_primary']
            self.anomalies_val.setStyleSheet(f"color: {color}; font-size: 18px; font-weight: 800; border: none; background: transparent;")
        if duration is not None:
            self.duration_secs = duration
            if duration < 60: self.duration_val.setText(f"{duration}s")
            elif duration < 3600: self.duration_val.setText(f"{duration//60}m {duration%60}s")
            else: self.duration_val.setText(f"{duration//3600}h {(duration%3600)//60}m")
...
class AnalysisWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # e.unsatisfied is a dict of unsatisfied requirements
            missing_keys = list(getattr(e, 'unsatisfied', {}).keys())
            missing_str = ", ".join(missing_keys) if missing_keys else str(e)
            self.log_message.emit(f"Missing required parameter(s): {missing_str}")
            self.analysis_error.emit(self.plugin_name, f"Missing required parameter(s): {missing_str}\n\nPlease check the Plugin Configuration panel.")
    ...
    def _setup_ui(self):
        ...
        def _on_mode_change(idx):
            if idx == 0:
                ...
                self.target_input.setPlaceholderText(r"e.g. password\.txt or \.pdf$")
...
class ResultsTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Anomaly badge (like reference: "X ANOMALIES DETECTED")
        if self._is_process_view:
            ...
            if suspicious_count > 0:
                ...
                badge = QLabel(f"  🔴  {suspicious_count} ANOMAL{'IES' if suspicious_count > 1 else 'Y'} DETECTED  ")
                badge.setStyleSheet(f"background: {COLORS['error']}; color: white; border-radius: 6px; padding: 6px 16px; font-weight: 800; font-size: 11px; letter-spacing: 1px;")
                badge_row.addWidget(badge)
                badge_detail = QLabel(f"  {suspicious_count} suspicious process{'es' if suspicious_count > 1 else ''} highlighted in results")
                badge_detail.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 11px;")
                badge_row.addWidget(badge_detail)
                badge_row.addStretch()
                layout.addLayout(badge_row)

        # Process detail panel (shows selected row info)
        self.detail_panel = QWidget()
        self.detail_panel.setStyleSheet(f"background: {COLORS['bg_surface']}; border: 1px solid {COLORS['border']}; border-radius: 10px;")
        self.detail_panel.setFixedHeight(0)  # Start hidden
        self.detail_panel.setVisible(False)
        dp_layout = QVBoxLayout(self.detail_panel); dp_layout.setContentsMargins(14, 10, 14, 10); dp_layout.setSpacing(4)
        self.detail_title = QLabel("PROCESS DETAILS"); self.detail_title.setStyleSheet(f"color: {COLORS['primary']}; font-size: 11px; font-weight: 800; letter-spacing: 1.5px;")
        dp_layout.addWidget(self.detail_title)
        self.detail_content = QLabel(""); self.detail_content.setWordWrap(True)
        self.detail_content.setStyleSheet(f"color: {COLORS['text_secondary']}; font-size: 12px; font-family: Consolas, monospace;")
        dp_layout.addWidget(self.detail_content)
        layout.addWidget(self.detail_panel)

        # Empty state
        if len(self.rows) == 0:
            empty = QLabel("  📭  No results returned\n\n  The plugin executed successfully but found no matching data.")
    ...
    def _on_selection_changed(self):
        ...
        if count == 1:
            ...
            for ci in range(self.table.columnCount()):
                ...
                col_name = self.friendly_columns[ci] if ci < len(self.friendly_columns) else f"Col {ci}"
                if ci == self._name_col: name_val = val
                if ci == self._pid_col: pid_val = val
                color = COLORS['accent'] if ci == self._name_col else COLORS['text_secondary']
                parts.append(f'<b style="color:{COLORS["text_muted"]}">{col_name}:</b> <span style="color:{color}">{val}</span>')
            header = f"→ {name_val}" + (f"  [PID {pid_val}]" if pid_val else "")
            self.detail_title.setText(f"PROCESS DETAILS   {header}")
            # Show as grid: 4 items per row
            html_rows = []
            for i in range(0, min(len(parts), 12), 4):
                chunk = parts[i:i+4]
                html_rows.append("  &nbsp;&nbsp;│&nbsp;&nbsp;  ".join(chunk))
            self.detail_content.setText("\n".join(html_rows))
            self.detail_panel.setVisible(True); self.detail_panel.setFixedHeight(90)
        elif count == 0:
            self.detail_panel.setVisible(False); self.detail_panel.setFixedHeight(0)
...
class WelcomeWidget(QWidget):
    ...
    def _setup_ui(self):
        layout = QVBoxLayout(self); layout.setContentsMargins(30,15,30,10); layout.setSpacing(4)
        layout.addStretch(1)
        # Logo + Title in one row
        header = QHBoxLayout(); header.setAlignment(Qt.AlignmentFlag.AlignCenter); header.setSpacing(10)
        self.logo = QLabel("🔬"); self.logo.setStyleSheet("font-size: 32px; background: transparent;")
        self._logo_opacity = QGraphicsOpacityEffect(self.logo); self.logo.setGraphicsEffect(self._logo_opacity)
        self._pulse_anim = QPropertyAnimation(self._logo_opacity, b"opacity")
        self._pulse_anim.setDuration(2500); self._pulse_anim.setStartValue(0.6); self._pulse_anim.setEndValue(1.0)
        self._pulse_anim.setEasingCurve(QEasingCurve.Type.InOutSine); self._pulse_anim.setLoopCount(-1)
        self._pulse_anim.start()
        header.addWidget(self.logo)
        title = QLabel("VOLATILITY 3"); title.setStyleSheet(f"color: {COLORS['primary']}; font-size: 22px; font-weight: 800; letter-spacing: 3px; background: transparent;")
        header.addWidget(title)
        ver = QLabel(f"v{APP_VERSION}"); ver.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 9px; font-weight: 700; background: {COLORS['bg_surface']}; border: 1px solid {COLORS['border']}; border-radius: 8px; padding: 2px 7px;")
        header.addWidget(ver)
        layout.addLayout(header)
        sub = QLabel("Professional Memory Forensic Workstation"); sub.setAlignment(Qt.AlignmentFlag.AlignCenter)
        sub.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 12px; background: transparent;"); layout.addWidget(sub)
        layout.addSpacing(14)
        # Open button
        ob = QPushButton("📂  Open Memory Image"); ob.setMinimumHeight(44); ob.setMaximumWidth(320)
        ob.setStyleSheet(f"QPushButton {{ background: qlineargradient(x1:0,y1:0,x2:1,y2:0,stop:0 {COLORS['primary_dim']},stop:1 {COLORS['primary']}); color: {COLORS['bg_darkest']}; border: none; font-weight: 800; font-size: 14px; padding: 12px 36px; border-radius: 10px; }} QPushButton:hover {{ background: qlineargradient(x1:0,y1:0,x2:1,y2:0,stop:0 {COLORS['primary']},stop:1 {COLORS['accent']}); }}")
        ...
        dh.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 10px; background: transparent;"); layout.addWidget(dh)
        layout.addSpacing(8)
        # Shortcut chips
        chips_row = QHBoxLayout(); chips_row.setAlignment(Qt.AlignmentFlag.AlignCenter); chips_row.setSpacing(6)
        chip_style = f"color: {COLORS['text_muted']}; font-size: 9px; font-weight: 600; background: {COLORS['bg_surface']}; border: 1px solid {COLORS['border']}; border-radius: 5px; padding: 3px 8px;"
        for txt in ["Ctrl+O", "F5 Run", "Ctrl+D Hash", "F1 Help"]:
            chip = QLabel(txt); chip.setStyleSheet(chip_style); chips_row.addWidget(chip)
        layout.addLayout(chips_row)
        layout.addSpacing(10)
        # Gradient divider
        divider = QFrame(); divider.setFixedHeight(1)
        divider.setStyleSheet(f"background: qlineargradient(x1:0,y1:0,x2:1,y2:0,stop:0 transparent,stop:0.3 {COLORS['border_light']},stop:0.7 {COLORS['border_light']},stop:1 transparent);")
        layout.addWidget(divider)
        layout.addSpacing(6)
        # Recent files
        self.recent_container = QWidget(); self.recent_container.setStyleSheet("background: transparent;")
        self.recent_layout = QVBoxLayout(self.recent_container); self.recent_layout.setContentsMargins(0,0,0,0); self.recent_layout.setSpacing(3)
...
class PluginConfigForm(QWidget):
    ...
    def _create_widget(self, req):
        if isinstance(req, requirements.BooleanRequirement):
            ...
            le = QLineEdit(); le.setPlaceholderText("Enter number (e.g. 1234 or 0xABCD)")
            if req.default is not None: le.setText(str(req.default))
            return le
    ...
    def get_config(self):
        ...
        for name, (w, req) in self.config_widgets.items():
            ...
            if isinstance(req, requirements.BooleanRequirement): val = w.isChecked()
                ...
                if t:
                    try:
                        val = int(t, 16) if t.lower().startswith("0x") else int(t)
                    except ValueError:
...
class VolatilityGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(APP_NAME); self.setMinimumSize(1024, 768); self.resize(1280, 800)
        ...
        self._total_results = 0; self._total_anomalies = 0; self._total_duration = 0
        self._spinner = SpinnerAnimation()
        self._spinner_timer = QTimer(); self._spinner_timer.timeout.connect(self._update_spinner)
    ...
    def _create_statusbar(self):
        ...
        pill_style = f"background: {COLORS['bg_surface']}; border: 1px solid {COLORS['border']}; border-radius: 10px; padding: 3px 10px; font-size: 11px; font-weight: 600;"
        self.status_elapsed = QLabel(""); self.status_elapsed.setStyleSheet(f"color: {COLORS['text_muted']}; {pill_style} font-family: Consolas, monospace;"); sb.addPermanentWidget(self.status_elapsed)
        self.status_activity = QLabel(""); self.status_activity.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 16px;"); sb.addPermanentWidget(self.status_activity)
        self.status_mem = QLabel(""); self.status_mem.setStyleSheet(f"color: {COLORS['text_muted']}; {pill_style}"); sb.addPermanentWidget(self.status_mem)
        self.status_plugins = QLabel(f"  ⚡ {len(self.plugin_list)} plugins  "); self.status_plugins.setStyleSheet(f"color: {COLORS['primary']}; {pill_style}"); sb.addPermanentWidget(self.status_plugins)
        self.status_clock = QLabel(""); self.status_clock.setStyleSheet(f"color: {COLORS['text_muted']}; {pill_style} font-family: Consolas, monospace;"); sb.addPermanentWidget(self.status_clock); self._update_clock()
    ...
    def _update_memory(self):
        try:
            ...
            if mb < 500: color = COLORS['success']
            elif mb < 1024: color = COLORS['warning']
            else: color = COLORS['error']
            self.status_mem.setText(f"💾 {mb:.0f} MB")
            pill = f"background: {COLORS['bg_surface']}; border: 1px solid {COLORS['border']}; border-radius: 10px; padding: 3px 10px; font-size: 11px; font-weight: 600;"
            self.status_mem.setStyleSheet(f"color: {color}; {pill}")
        except: pass
    ...
    def _create_ui(self):
        ...
        ml.addWidget(ib)
        # Main splitter
        self.main_splitter = QSplitter(Qt.Orientation.Horizontal); self.main_splitter.setHandleWidth(3)
        # ─── LEFT SIDEBAR: Plugins + Config + Controls + Stats ───
        lp = QWidget(); lp.setMinimumWidth(240); lp.setMaximumWidth(340)
        ll = QVBoxLayout(lp); ll.setContentsMargins(8,8,4,8); ll.setSpacing(5)
        # Plugin header
        lph = QHBoxLayout()
        ...
        ll.addWidget(self.plugin_tree, 1); self._populate_plugin_tree()
        # ─── Sidebar separator ───
        sep1 = QFrame(); sep1.setFixedHeight(1); sep1.setStyleSheet(f"background: {COLORS['border']};"); ll.addWidget(sep1)
        # ─── Plugin Config (in sidebar) ───
        cfg_title = QLabel("PLUGIN"); cfg_title.setObjectName("sectionTitle"); ll.addWidget(cfg_title)
        self.config_form = PluginConfigForm()
        self.config_form.setMaximumHeight(120)
        ll.addWidget(self.config_form)
        # ─── Run / Stop controls ───
        ctrl = QHBoxLayout(); ctrl.setSpacing(4)
        self.run_btn = QPushButton("▶  Run Analysis"); self.run_btn.setObjectName("runButton"); self.run_btn.clicked.connect(self._run_analysis); self.run_btn.setEnabled(False); self.run_btn.setMinimumHeight(36); ctrl.addWidget(self.run_btn)
        self.stop_btn = QPushButton("⏹"); self.stop_btn.setObjectName("stopButton"); self.stop_btn.clicked.connect(self._stop_analysis); self.stop_btn.setEnabled(False); self.stop_btn.setFixedSize(36, 36); ctrl.addWidget(self.stop_btn)
        ll.addLayout(ctrl)
        self.progress_bar = QProgressBar(); self.progress_bar.setRange(0,100); self.progress_bar.setValue(0); self.progress_bar.setTextVisible(True); self.progress_bar.setFormat("%p%"); self.progress_bar.setFixedHeight(16); ll.addWidget(self.progress_bar)
        self.progress_label = QLabel(""); self.progress_label.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 10px;"); self.progress_label.setWordWrap(True); ll.addWidget(self.progress_label)
        # ─── Sidebar separator ───
        sep2 = QFrame(); sep2.setFixedHeight(1); sep2.setStyleSheet(f"background: {COLORS['border']};"); ll.addWidget(sep2)
        # ─── Session Stats ───
        self.session_stats = SessionStatsWidget()
        ll.addWidget(self.session_stats)
        self.main_splitter.addWidget(lp)
        # ─── CENTER: Results + Bottom Panels ───
        cp = QWidget(); cl = QVBoxLayout(cp); cl.setContentsMargins(0,4,4,4); cl.setSpacing(3)
        cs = QSplitter(Qt.Orientation.Vertical)
        ...
        cs.setStretchFactor(0, 3); cs.setStretchFactor(1, 1)
        cs.setSizes([580, 160])
        cl.addWidget(cs); self.main_splitter.addWidget(cp)
        self.main_splitter.setSizes([270, 750])
    ...
    def _run_analysis(self):
        ...
        self.progress_bar.setValue(0); self._spinner.reset()
        self.progress_label.setText(f"{self._spinner.next()} Running {friendly}..."); self.progress_label.setStyleSheet(f"color: {COLORS['primary']}; font-size: 11px; font-weight: 600;")
        ...
        self._start_time = datetime.datetime.now(); self._elapsed_timer.start(1000); self._spinner_timer.start(100)
        ...
        self.session_stats.update_stats(runs=self._analysis_count)
    ...
    def _on_finished(self, pn, cols, rows, hi):
        self._elapsed_timer.stop(); self._spinner_timer.stop(); elapsed = self._get_elapsed()
        friendly = get_plugin_friendly_name(pn)
        ...
        self.progress_label.setText(f"✅ {friendly} — {len(rows)} results in {elapsed}")
        self.progress_label.setStyleSheet(f"color: {COLORS['success']}; font-size: 11px; font-weight: 600;")
        hi.setText(2, f"✅ {len(rows)} rows"); hi.setForeground(2, QColor(COLORS['success'])); hi.setText(3, elapsed)
        # Count anomalies (suspicious processes) in results
        name_col = -1
        for ci2, c in enumerate(cols):
            if c.lower() in ('imagefilename', 'process', 'name', 'comm'): name_col = ci2; break
        anomalies_in_run = 0
        if name_col >= 0:
            for row in rows:
                if name_col < len(row) and is_process_suspicious(str(row[name_col])):
                    anomalies_in_run += 1
        self._total_anomalies += anomalies_in_run
        self._total_results += len(rows)
        elapsed_secs = int((datetime.datetime.now() - self._start_time).total_seconds()) if self._start_time else 0
        self._total_duration += elapsed_secs
        self.session_stats.update_stats(results=self._total_results, anomalies=self._total_anomalies, duration=self._total_duration)
        tab = ResultsTab(pn, cols, rows)
        tab_label = f"📊 {friendly} ({len(rows)})"
        idx = self.results_tabs.addTab(tab, tab_label); self.results_tabs.setCurrentIndex(idx)
    ...
    def _on_error(self, pn, err, hi):
        self._elapsed_timer.stop(); self._spinner_timer.stop(); elapsed = self._get_elapsed(); friendly = get_plugin_friendly_name(pn)
        self.run_btn.setEnabled(True); self.stop_btn.setEnabled(False); self.toolbar_run.setEnabled(True); self.toolbar_stop.setEnabled(False)
    ...
    def _update_spinner(self):
        """Update the progress label with a spinning animation character."""
        txt = self.progress_label.text()
        if txt and self.current_worker:
            try:
                if self.current_worker.isRunning():
                    # Extract the text after the first character (spinner)
                    core = txt[1:].strip() if len(txt) > 1 and ord(txt[0]) > 0x2800 else txt
                    self.progress_label.setText(f"{self._spinner.next()} {core}")
            except RuntimeError: pass
